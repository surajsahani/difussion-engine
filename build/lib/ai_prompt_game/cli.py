...
def show_stats():
    ...
    if progress:
        ...
        completed = overall.get('completed_challenges', 0)
        total = overall.get('total_challenges', 6)
        percentage = overall.get('completion_percentage', 0)
        
        print(f"🎯 Challenge Progress: {completed}/{total} ({percentage:.1f}%)")
        ...
        # Define challenge order and info
        challenges = [
            {'name': 'cat', 'title': 'Cat', 'difficulty': 'Easy', 'threshold': 0.65},
            {'name': 'coffee', 'title': 'Coffee', 'difficulty': 'Easy', 'threshold': 0.65},
            {'name': 'car', 'title': 'Car', 'difficulty': 'Medium', 'threshold': 0.60},
            {'name': 'foxes', 'title': 'Foxes', 'difficulty': 'Medium', 'threshold': 0.60},
            {'name': 'llama', 'title': 'Llama', 'difficulty': 'Medium', 'threshold': 0.60},
            {'name': 'owl', 'title': 'Owl', 'difficulty': 'Hard', 'threshold': 0.55}
        ]
        ...
        for i, challenge in enumerate(challenges, 1):
            ...
            if name in progress and progress[name].get('completed', False):
                score = progress[name]['best_score']
                attempts = progress[name]['attempts_to_complete']
                status = "✅"
                details = f"Score: {score:.3f} (in {attempts} attempts)"
            else:
                status = "🔒" if i > completed + 1 else "📝"
                details = f"Required: {threshold:.3f} similarity"
            
            print(f"   {i}. {status} {title} ({difficulty}) - {details}")
        
        if completed > 0:
            print(f"\n🎉 Congratulations! You've completed {completed} challenge{'s' if completed != 1 else ''}!")
        
        if completed < total:
            next_challenge = challenges[completed]['title']
            print(f"\n🎯 Next Challenge: {next_challenge}")
            print(f"💡 Play it: ai-prompt-game --target {challenges[completed]['name']}")
    
    else:
        print("📈 No progress yet!")
        print("💡 Start your first challenge: ai-prompt-game --target cat")
        return
    
    # Show legacy stats if available
    ...
    if stats:
        ...
        if 'recent_scores' in stats:
            ...
            print(f"   📊 Recent Scores: {[f'{s:.3f}' for s in recent]}")
