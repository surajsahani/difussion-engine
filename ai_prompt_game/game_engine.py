...
class PromptGame:
    ...
    def make_attempt(self, prompt):
        ...
        passing_result = self.check_passing_criteria(combined_score, self.current_target)
        if passing_result['passed']:
            print(f"\n🎉 {passing_result['message']}")
            print(f"🎯 You've mastered this challenge!")
            if passing_result['unlock_next']:
                print(f"🔓 Next challenge unlocked: {passing_result['next_challenge']}")
            print(f"📈 Your progress has been saved!")
            
            # Save progress
            self.save_progress(self.current_target['name'], combined_score, attempt_num)
            
            # Ask if they want to continue to next challenge
            if passing_result['unlock_next']:
                try:
                    continue_choice = input(f"\n🚀 Continue to {passing_result['next_challenge']}? (y/n): ").lower().strip()
                    if continue_choice in ['y', 'yes']:
                        self.play_target(passing_result['next_target_key'])
                        return result
    ...
    def check_passing_criteria(self, score, target):
        """Check if the student has passed the current challenge"""
        
        # Define passing thresholds based on difficulty
        difficulty_thresholds = {
            'Easy': 0.65,      # 65% similarity for easy targets
            'Medium': 0.60,    # 60% similarity for medium targets  
            'Hard': 0.55       # 55% similarity for hard targets
        }
        
        difficulty = target.get('difficulty', 'Medium')
        threshold = difficulty_thresholds.get(difficulty, 0.60)
        
        passed = score >= threshold
        
        if passed:
            # Define challenge progression order
            challenge_order = ['cat', 'coffee', 'car', 'foxes', 'llama', 'owl']
            current_index = challenge_order.index(target['name']) if target['name'] in challenge_order else -1
            
            unlock_next = current_index >= 0 and current_index < len(challenge_order) - 1
            next_target_key = challenge_order[current_index + 1] if unlock_next else None
            next_challenge = next_target_key.title() if next_target_key else None
            
            messages = [
                f"CONGRATULATIONS! Score: {score:.3f} (Required: {threshold:.3f})",
                f"EXCELLENT WORK! You achieved {score:.3f} similarity!",
                f"CHALLENGE COMPLETED! Outstanding {score:.3f} score!",
                f"WELL DONE! You mastered this with {score:.3f} similarity!"
            ]
            ...
            percentage = (score / threshold) * 100
            
            return {
                'passed': False,
                'message': f"Keep trying! You need {remaining:.3f} more points ({percentage:.1f}% there)",
                'threshold': threshold,
                'unlock_next': False,
                'next_challenge': None,
    ...
    def save_progress(self, target_name, score, attempts):
        """Save student progress to file"""
        try:
            progress_file = self.game_dir / "progress.json"
            ...
            if target_name not in progress:
                progress[target_name] = {}
            
            progress[target_name].update({
                'completed': True,
                'best_score': score,
                'attempts_to_complete': attempts,
                'completion_date': datetime.now().isoformat(),
                'difficulty': self.current_target.get('difficulty', 'Medium')
            })
            
            # Calculate overall progress
            total_targets = 6  # cat, coffee, car, foxes, llama, owl
            completed_targets = len([t for t in progress.values() if t.get('completed', False)])
            progress['overall'] = {
                'completed_challenges': completed_targets,
                'total_challenges': total_targets,
            ...
            if self.verbose:
                print(f"📊 Progress saved: {completed_targets}/{total_targets} challenges completed")
